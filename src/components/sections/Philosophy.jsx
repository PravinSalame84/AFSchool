...
const pillars = [
  ...
  {
    icon: ShieldCheck,
    title: 'Values Before Vanity',
    description: 'The school places character, discipline, humility and responsible conduct alongside academic progress.',
  },
  {
    icon: HeartHandshake,
    title: 'Every Child Participates',
    description: 'Assemblies, clubs, cultural programs, sports and annual events are used to help each child feel seen, prepared and involved.',
  },
]
      ...
      <Container sx={{ px: { xs: 2, sm: 3, lg: 4 }, textAlign: 'center' }}>
        <RevealOnScroll>
          <Typography sx={{ mx: 'auto', maxWidth: 760, color: BRAND_NEUTRALS.white, fontSize: { xs: '1.8rem', sm: '2.2rem' }, fontWeight: 700, lineHeight: 1.3 }}>
            ...
            <OptimizedImage src={sharedImages.teacherImageTwo} alt="About Snapshot" sx={{ width: '100%', maxWidth: 750, my: 3, borderRadius: '1rem', boxShadow: 5, border: `2px solid ${BRAND_NEUTRALS.white}` }} />
            {/* <OptimizedImage src={campusImages.teacherMusic} alt="Music learning at Air Force School" sx={{ width: '100%', maxWidth: 750, mb: 3, borderRadius: 1, boxShadow: 5 }} /> */}
            {schoolContent.hero.motto}{' '}
            <Box component="span" sx={{ color: 'secondary.main' }}>guides the way children learn, perform and grow.</Box>
          </Typography>
        </RevealOnScroll>

        <Box sx={{ mx: 'auto', mt: 6, maxWidth: 960, display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(3, 1fr)' }, gap: 3 }}>
          {pillars.map((p, i) => (
            <RevealOnScroll key={p.title} delay={i * 120}>
              <Box sx={{ borderRadius: 1, bgcolor: 'rgba(255,255,255,0.05)', p: 3.5, textAlign: 'left', border: `2px solid ${BRAND_NEUTRALS.white}` }}>
                ...
                <Typography sx={{ mt: 1.25, color: `${BRAND_NEUTRALS.white}`, fontSize: '0.9rem', lineHeight: 1.8 }}>{p.description}</Typography>
              </Box>
            </RevealOnScroll>
          ))}
        </Box>
