...
import { ArrowDown, BookOpenCheck, GraduationCap, ShieldCheck, Sparkles } from 'lucide-react'
...
export default function Hero() {
  ...
  return (
    <section className="relative overflow-hidden px-4 pb-16 pt-6 sm:px-6 lg:px-8 lg:pb-20">
      ...
      <div className="pointer-events-none absolute right-[10%] top-24 hidden h-80 w-80 rounded-full border border-primary-900/10 lg:block animate-spinSlow" />
      ...
      <Container className="relative">
        <div className="grid grid-cols-1 gap-10 xl:grid-cols-[92px_1.05fr_0.95fr] xl:items-center">
          <div className="hidden xl:flex xl:flex-col xl:items-center xl:gap-8">
            <div className="rounded-full border border-primary-900/10 bg-white/60 p-3 text-primary-700 shadow-soft">
              <Sparkles className="h-5 w-5" />
            </div>
            <div className="hero-index text-sm font-bold uppercase tracking-[0.55em] text-primary-400">
              01 02 03
            </div>
          ...
          <div>
            <motion.div initial="hidden" animate="show" custom={0} variants={fadeUp}>
              <span className="inline-flex items-center gap-2 rounded-full border border-white/70 bg-white/72 px-4 py-2 text-xs font-bold uppercase tracking-[0.22em] text-primary-700 shadow-soft backdrop-blur-xl">
            ...
            <motion.div initial="hidden" animate="show" custom={0.08} variants={fadeUp}>
              <div className="mt-7 flex items-start gap-5">
                ...
                <h1 className="max-w-3xl text-[3.15rem] font-bold uppercase leading-[0.88] text-primary-900 sm:text-[4.6rem]">
                  Air Force
                  <span className="block text-primary-500">School</span>
                  <span className="block text-[0.56em] font-medium uppercase tracking-[0.14em] text-primary-500">
            ...
            <motion.p
              ...
              custom={0.16}
              variants={fadeUp}
              className="mt-6 max-w-2xl text-lg leading-relaxed text-primary-600"
            >
            ...
            >
              {schoolContent.hero.badges.map((badge) => (
                <span key={badge} className="rounded-full border border-primary-900/10 bg-white/72 px-4 py-2 text-xs font-bold uppercase tracking-[0.16em] text-primary-700 shadow-soft">
                  {badge}
                </span>
              ))}
            </motion.div>
          ...
          >
            ...
            <div className="student-mask relative overflow-hidden rounded-[2.8rem] border border-white/70 bg-white/40 p-4 shadow-card backdrop-blur-xl">
              <img
                src="https://www.airforce.skoolmate.in/wp-content/uploads/2023/11/DSC00069-scaled-e1700192856657.jpg"
                alt="Air Force School students"
                className="h-[420px] w-full rounded-[2.2rem] object-cover object-center"
              />
            </div>
            <div className="absolute -left-3 bottom-10 frost-card rounded-[1.75rem] p-4 shadow-card">
              <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-primary-400">Motto</p>
              <p className="mt-1 text-lg font-bold uppercase text-primary-900">{schoolContent.hero.motto}</p>
            </div>
            <div className="absolute -right-2 top-12 rounded-full bg-primary-900 px-5 py-3 text-xs font-bold uppercase tracking-[0.2em] text-white shadow-card">
              CBSE 1130860
            </div>
          </motion.div>
        </div>
            ...
            return (
              ...
              >
                <div className={`relative overflow-hidden rounded-[1.6rem] bg-gradient-to-br ${card.accent} p-5 text-white`}>
                  ...
                  <img src={card.image} alt={card.title} className="mt-5 h-44 w-full rounded-[1.2rem] object-cover shadow-card" />
                </div>
                <p className="px-2 pb-2 pt-4 text-sm leading-relaxed text-primary-600">{card.description}</p>
              </motion.article>
            )
          })}
        </div>
